...
public class Player extends Actor {
   ...
   /** Crouched movement multiplier. */
    public float crouchSpeedMultiplier = 0.45f;

   /** Standing collision height. */
    public float standingCollisionHeight = 0.65f;

   /** Crouched collision height. */
   public float crouchingCollisionHeight = 0.40f;

  /** Normal camera eye height. */
   public float standingEyeHeight = 0.12f;

  /** Camera eye height while crouched. */
  public float crouchingEyeHeight = -0.08f;

   /** Current crouch state. */
  public transient boolean isCrouching = false;

  /** Current sprint state. */
   public transient boolean isSprinting = false;
    ...
    public void setMaxMana(int amount, boolean refill) {
        maxMp = Math.max(0, amount);
    ...
    private void tickManaRegen(float delta) {
        if(!manaRegenerationEnabled) {
        ...
        manaRegenAccumulator += manaRegenRate * delta;

        while(manaRegenAccumulator >= 1f && mp < maxMp) {
            mp++;
	...
	public void tick(Level level, float delta, GameInput input) {
		...
		boolean up = false, down = false, left = false, right = false, turnLeft = false, turnRight = false, turnUp = false, turnDown = false, attack = false, jump = false, sprint = false, crouch = false;
        ...
        if(crouch) {
            ...
            collision.z = crouchingCollisionHeight;
            eyeHeight = crouchingEyeHeight;
        }
        else if(isCrouching) {
            // Before standing up, verify that the full standing collider fits.
            float oldCollisionHeight = collision.z;

            collision.z = standingCollisionHeight;

            boolean canStand = level.isFree(
                x,
                y,
                z,
                collision,
            ...
            if(canStand) {
                ...
                // Ceiling is too low. Stay crouched.
                collision.z = oldCollisionHeight;
                eyeHeight = crouchingEyeHeight;
            }
        }
        else {
        ...
        // Crouching always overrides sprinting.
        if(isCrouching) {
            isSprinting = false;
            walkVel *= crouchSpeedMultiplier;
        }
        else {
            // Only sprint while actually moving forward.
            isSprinting = sprint && up && !down;

            if(isSprinting) {
                walkVel *= sprintSpeedMultiplier;
            }
        }
		...
		if(jump && (isOnFloor || isOnEntity) && !isOnLadder && !isCrouching) {
			za += jumpHeight;
		}
