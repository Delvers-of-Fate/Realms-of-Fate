...
public class GlRenderer {
    ...
    protected TextureRegion manaBarTextureRegion;
    ...
    protected int lastDrawnMp = -1;
    protected int lastDrawnMaxMp = -1;
	...
	public void initTextures() {
        ...
        // Load the mana bar UI
        Texture manaBarTexture = Art.loadTexture("ui/manabar.png");
        manaBarTextureRegion = new TextureRegion(manaBarTexture);
	...
	protected void drawUi()
	{
        ...
        if(game.player.maxMp > 0) {
            ...
            float manaPadY = padY + (10f * healthUiSize);

            // Calculate how full the mana bar should be.
            float manaPercent = (float)game.player.mp / (float)game.player.maxMp;

            // Keep the value safely between 0 and 1.
            manaPercent = Math.max(0f, Math.min(1f, manaPercent));

            // Draw the blue mana fill.
            uiBatch.setColor(0.15f, 0.35f, 1f, 1f);

            uiBatch.draw(
                fontAtlas.getSprite(55),
                -camera2D.viewportWidth / 2 + padX * 1.75f,
                -camera2D.viewportHeight / 2 + manaPadY + barHeight - 4 * healthUiSize,
                Math.max(0f, (healthBarWidth - padX) * manaPercent),
                barHeight
            );

            // Draw another copy of the normal Delver bar frame.
            uiBatch.setColor(Color.WHITE);

            uiBatch.draw(
                healthBarTextureRegion,
                -camera2D.viewportWidth / 2 + padX,
                -camera2D.viewportHeight / 2 + manaPadY,
                healthBarWidth,
                healthBarHeight
            );

            // Only rebuild the mana text when MP changes.
            if(lastDrawnMp != game.player.mp || lastDrawnMaxMp != game.player.maxMp) {
                manaText = game.player.mp + "/" + game.player.maxMp;
                lastDrawnMp = game.player.mp;
                lastDrawnMaxMp = game.player.maxMp;
            }

            // Draw current / maximum mana.
            drawText(
                manaText,
                -camera2D.viewportWidth / 2f
                    + healthSize * 5.5f
                    - ((healthSize * 0.4f) * (manaText.length() - 1)),
                -camera2D.viewportHeight / 2f
                    + manaPadY
                    + barHeight * 0.6f,
                healthUiSize * 3.75f,
                Color.WHITE
